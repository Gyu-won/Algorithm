...
//        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            int dest = Integer.parseInt(st.nextToken());
            int time = Integer.parseInt(st.nextToken());
            distance[start][dest] = Math.min(distance[start][dest], time);
        }
        ...
        for (int i = 1; i <= n; i++) {
            if (i != x) {
                maxValue = Math.max(distance[i][x] + distance[x][i], maxValue);
            }
        }
...
//    private static int calculateMinDistance(int start, int dest){
//        PriorityQueue<Road> pq = new PriorityQueue<>(new RoadComparator());
